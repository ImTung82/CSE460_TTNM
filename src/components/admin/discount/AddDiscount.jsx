...
function AddDiscount() {
    ...
    const validateField = (name, value) => {
        ...
        switch (name) {
            case 'maGiamGia':
                if (!value || !value.trim()) error = 'Mã giảm giá không được bỏ trống';
                else if (value.trim().length > 50) error = 'Mã giảm giá không được vượt quá 50 ký tự';
                break;
            case 'hinhAnh':
                if (!value) {
            ...
            case 'giaTriGiam':
                if (!value || !value.toString().trim()) {
                    ...
                    error = 'Giá trị giảm chỉ chấp nhận số';
                } else if (parseFloat(value) < 1 || parseFloat(value) > 100) {
                    error = 'Giá trị giảm phải nằm trong khoảng từ 1 đến 100 (%)';
                }
                break;
            case 'ngayBatDau':
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(() => {
            navigate('/admin/giam-gia', { state: { message: 'Thêm mã giảm giá thành công!' } });
        }, 1000);
    };

    return (
        <div className="bg-white">
            <h2 className="text-2xl font-bold mb-6 text-black">Thêm mã giảm giá mới</h2>
                        ...
                        <img
                            src={
                                typeof formData.hinhAnh === "string"
                                    ? `/uploads/${formData.hinhAnh}` // ảnh cũ từ DB
                                    : URL.createObjectURL(formData.hinhAnh) // ảnh mới
                            }
                            alt="Preview"
