...
const UserTable = () => {
  ...
  const users = [
    { id: 'U001', name: 'Nguyễn Văn A', email: 'a@example.com', phone: '0912345678', address: 'Hà Nội', role: 'user' },
    { id: 'U002', name: 'Trần Thị B', email: 'b@example.com', phone: '0987654321', address: 'HCM', role: 'admin' },
  ];
  ...
  return (
    <div className="p-6 relative">  
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/admin/nguoi-dung/them-moi" className="flex items-center bg-[#28A745] hover:opacity-70 text-white font-bold px-4 py-3 rounded-md transition duration-200">
      ...
      <table className="min-w-full table-auto overflow-hidden">
        ...
        <tbody className="bg-white">
          {users.map((u, idx) => (
            <tr key={u.id} className="text-sm hover:bg-gray-50 leading-relaxed">
              <td className="border-t border-gray-300 px-4 py-4">{u.id}</td>
              <td className="border-t border-gray-300 px-4 py-4">{u.name}</td>
              <td className="border-t border-gray-300 px-4 py-4">{u.email}</td>
              <td className="border-t border-gray-300 px-4 py-4">{u.phone}</td>
              <td className="border-t border-gray-300 px-4 py-4">{u.address}</td>
              <td className="border-t border-gray-300 px-4 py-4">{u.role}</td>
              <td className="border-t border-gray-300 px-4 py-4">
                <div className="flex justify-center items-center gap-2">
                  <Link to="/admin/nguoi-dung/sua-nguoi-dung" className="text-yellow-500 hover:text-yellow-600">
                  ...
                  <button onClick={() => openDelete(u)} className="text-red-500 hover:text-red-600">
      ...
      {/* Modals */}
      <DeleteUserModal isOpen={isDeleteOpen} onClose={closeDelete} onConfirm={confirmDelete} user={userToDelete} />
    </div>
  );
};
